class Tip < ActiveRecord::Base
    ...
    if(Rails.env.production?)
        scope :ordered_by_vote_count, -> { joins(:votes).where("votes.weight > 0").select("COUNT(votes.id) - DATE_PART('days', NOW()::timestamp - tips.created_at::timestamp) as vote_count, tips.*").group('tips.id').order("vote_count DESC") }
    else
        scope :ordered_by_vote_count, -> { joins(:votes).where("votes.weight > 0").select("COUNT(votes.id) - (julianday('now') - julianday(tips.created_at)) as vote_count, tips.*").group('tips.id').order("vote_count DESC") }
    end
